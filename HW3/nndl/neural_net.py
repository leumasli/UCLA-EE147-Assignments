...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        data_loss = np.sum(true_probs) / N

        reg_loss = (np.sum(W1**2) + np.sum(W2**2))*reg/2
        ...
        grads["W2"] = dz2.T @ h1 + reg * W2
        grads["b2"] = dz2.T @ np.ones(N)

        dz1 = dz2 @ W2 * (h1 > 0)

        grads["W1"] = dz1.T @ X + reg * W1
        grads["b1"] = dz1.T @ np.ones(N)
