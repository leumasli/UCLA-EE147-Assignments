...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # ================================================================ #
        h1, cache1 = affine_relu_forward(
            X, self.params['W1'], self.params['b1'])
        scores, cache2 = affine_forward(
            h1, self.params['W2'], self.params['b2'])
        # ================================================================ #
        # END YOUR CODE HERE
        # ================================================================ #
        ...
        for i in range(1, self.num_layers+1):
            ...
            if self.use_batchnorm:
                if i != self.num_layers:
                    gammai = 'gamma' + str(i)
                    betai = 'beta' + str(i)
                    self.params[gammai] = np.ones(hidden_dims[i-1], )
                    self.params[betai] = np.zeros(hidden_dims[i-1], )
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            if self.use_batchnorm:
                gammai = 'gamma' + str(i)
                betai = 'beta' + str(i)
                h, fc_cache = affine_forward(
                    h, self.params[Wi], self.params[bi])
                h, bn_cache = batchnorm_forward(
                    h, self.params[gammai], self.params[betai], self.bn_params[i-1])
                h, relu_cache = relu_forward(h)
                cache[ci] = (fc_cache, bn_cache, relu_cache)
            else:
                h, cache[ci] = affine_relu_forward(
                    h, self.params[Wi], self.params[bi])
        ...
        scores, cache['c' + str(self.num_layers)] = affine_forward(h, self.params['W' + str(self.num_layers)],
        ...
        for i in range(self.num_layers-1, 0, -1):
            ...
            loss += np.sum(self.params[Wi]**2) * self.reg / 2
        # ================================================================ #
        # END YOUR CODE HERE
        # ================================================================ #
